...
exports.vehiclesInfoObject = gmResData => {
  gmResData = replaceSmartcarKeysInGMObject(gmResData, mockSmartcarResponses.vehiclesInfo);
...
exports.doorSecObject = gmResData => {
  gmResData = replaceSmartcarKeysInGMObject(gmResData, mockSmartcarResponses.securityObject);
...
exports.energyRangeObject = ( gmResData, energyType ) => {
  gmResData = replaceSmartcarKeysInGMObject(gmResData, mockSmartcarResponses.fuelBatteryObject);
  ...
  const gmActionType = energyTypeDict[energyType];
...
exports.engineActionObject = gmResData => {
  gmResData = replaceSmartcarKeysInGMObject(gmResData, mockSmartcarResponses.actionEngineObject);

  // conversions for gm result term to smartcar term
  const engineActionResultDict = {
    "EXECUTED": "success",
    "FAILED": "error"
  };

  const gmActionResultStatus = gmResData.actionResult.status;
...
const calcVehicleDoorCount = gmResData => {
  ...
  } else if( gmResData.twoDoorCoupe.value === 'True' ) {
    doorCount = 2;
...
const replaceSmartcarKeysInGMObject = ( gmRes, smartcarMockRes ) => {
